...
{{-- Intro Tutor --}}
<div class="container mt-4 px-8 lg:px-20">
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-4 items-center">
        ...
        <div class="space-y-4">
            ...
            {{-- Title --}}
            <h1 class="text-2xl lg:text-5xl font-bold text-gray-800">{{$subject->subject_title}}</h1>

            {{-- Tags --}}
            <div class="flex flex-wrap gap-2">
                @foreach(json_decode($subject->subject_majors) as $major)
                <span class="bg-gray-100 text-gray-800 py-1 px-2 rounded-xl text-sm lg:text-lg font-regular">{{$major}}</span>
                @endforeach
            </div>

            {{-- Category and Semester --}}
            <div class="flex flex-wrap items-center gap-3 text-gray-700">
                <p class="flex items-center text-sm lg:text-lg font-regular"><i class="fas fa-folder mr-2"></i> {{$subject->subject_category}}</p>
                <p class="flex items-center text-sm lg:text-lg font-regular"><i class="fas fa-gem mr-2"></i> {{$subject->subject_semester}}</p>
            </div>

            <x-button
                type="button"
                class="bg-orange-500 text-white p-2 lg:p-4 px-8 rounded-4 font-bold text-lg lg:text-2xl rounded-xl hover:scale-105 duration-300"
            ...
            <div class="space-y-6">
                <div>
                    ...
                    <div class="border-2 rounded-2xl mt-4 p-8 space-y-4">
                        <h3 class="text-xl lg:text-3xl font-extrabold">Tutor Background</h3>
                        <p class="text-m md:text-xl font-normal">Asal Universitas Tutor:</p>
                        ...
                        <a href="https://docs.google.com/spreadsheets/d/1rQ0ZhU6Ykdu4lrhhKMoHInTBQI2ywafQ0PnWIxZG-9w/edit?usp=sharing">
                            <x-button
                                type="button"
                                class="bg-lightblue text-black mt-4 p-4 px-4 rounded-4 font-regular text-xs lg:text-xl rounded-xl hover:scale-105 duration-300"
                                label="Click Here for Tutor Details"></x-button>
                        </a>

                        <h5 class="text-lg lg:text-xl font-semibold mt-4">Additional Information</h5>
                        <ul class="px-4 list-disc space-y-1 text-m lg:text-lg font-normal">
